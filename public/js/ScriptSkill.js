function submitSkill() {
    ...
    .then((response) => {
        ...
        $('#form-skill').trigger('reset').attr('method', 'post').attr('action', 'skill');
    ...
    .catch((error) => {
        ...
        if(error.response) {
            ...
            if(error.response.data.errors) {
                if(error.response.data.errors.name) { $('#alert-skill-name').html(error.response.data.errors.name).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.proficiency) { $('#alert-skill-proficiency').html(error.response.data.errors.proficiency).removeClass('d-none').hide().fadeIn('slow'); }
            }
        }
    });
};
...
const editSkill = (skill_id, item_id) => {
    $('#form-skill').attr('method', 'put').attr('action', 'skill/'+skill_id);
...
const deleteSkill = (skill_id) => {
  Swal.fire({
    ...
    if(result.isConfirmed) {
        var config = {
            method: 'delete', url: domain + 'skill/' + skill_id,
        }
        axios(config)
        .then((response) => {
            fetchSkill();
