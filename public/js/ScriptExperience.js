...
const newExp = () => {
    $('#form-exp').trigger('reset').attr('method', 'post').attr('action', 'experience');
    $('#exp-btn-submit').html("<i class='bx bxs-save me-2' ></i>Save</button>");
}
const editExp = (wh_id, item_id) => {
    $('.alert').hide();
    $('#form-exp').attr('method', 'put').attr('action', 'experience/'+wh_id);
    ...
    .catch((error) => {
        ...
        if(error.response) {
            ...
            if(error.response.data.errors) {
                if(error.response.data.errors.year_start) { $('#alert-exp-year_start').html(error.response.data.errors.year_start).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.year_end) { $('#alert-exp-year_end').html(error.response.data.errors.year_end).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.title) { $('#alert-exp-title').html(error.response.data.errors.title).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.affiliation) { $('#alert-exp-affiliation').html(error.response.data.errors.affiliation).removeClass('d-none').hide().fadeIn('slow'); }
            }
        }
    });
};
const deleteExp = (exp_id) => {
  Swal.fire({
    ...
    if(result.isConfirmed) {
        var config = {
            method: 'delete', url: domain + 'experience/' + exp_id,
        }
        axios(config)
        .then((response) => {
            fetchExp();
    ...
    .then((response) => {
        ...
        if(response.data.experiences.length > 0) {
            ...
            function foreachExp(item, index) {
                index++; let description = (item.description) ? item.description : '';
                let affiliation = (item.affiliation) ? item.affiliation : '';
                ...
                $('#container-experiences').append(`
                    <div class="exp-item d-flex justify-content-between mb-3">
                        <div class="col-mb-2">
                            ...
                            <p class="mb-1 fs-11 d-flex align-items-center"><span id="exp-affiliation-`+index+`" class="me-2">`+ affiliation +`</span> </p>
                            <p id="exp-description-`+index+`" class="fs-9 text-secondary mb-2">`+ description +`</p>
                        </div>
                        <div class="mb-3">
            ...
            $('#container-experiences').html(`<span class="fs-11 text-muted fst-italic">Tekan tombol simpan setelah mengisi data</span>`);
        }
    })
    .catch((error) => {
        console.log(error);
...
function submitWH() {
    ...
    .then((response) => {
        successMessage(response.data.message);
        $('#form-wh').trigger('reset').attr('method', 'post').attr('action', 'work_history');
    ...
    .catch((error) => {
        ...
        if(error.response) {
            ...
            if(error.response.data.errors) {
                if(error.response.data.errors.year_start) { $('#alert-wh-year_start').html(error.response.data.errors.year_start).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.year_end) { $('#alert-wh-year_end').html(error.response.data.errors.year_end).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.role) { $('#alert-wh-role').html(error.response.data.errors.role).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.work_place) { $('#alert-wh-work_place').html(error.response.data.errors.work_place).removeClass('d-none').hide().fadeIn('slow'); }
                if(error.response.data.errors.employment) { $('#alert-wh-employment').html(error.response.data.errors.employment).removeClass('d-none').hide().fadeIn('slow'); }
            }
        }
    });
};
    ...
    .then((response) => {
        ...
        if(response.data.work_histories.length > 0) {
            ...
            function foreachWH(item, index) {
                index++; let description = (item.description) ? item.description : '';
                let work_place = (item.work_place) ? item.work_place : '';
                var year_end = (item.year_end != null) ? item.year_end : 'Now';
                $('#container-work_histories').append(`
                    <div class="wh-item d-flex flex-remove-md justify-content-between mb-3">
                        <div class="col mb-2">
                            ...
                            <p class="mb-1 fs-11 d-flex align-items-center"><span id="wh-work_place-`+index+`" class="me-2">`+ work_place +`</span> <span id="wh-employment-`+index+`" class="badge bg-primary">`+item.employment+`</span></p>
                            <p id="wh-description-`+index+`" class="fs-9 text-secondary mb-2">`+description+`</p>
                        </div>
                        <div class="mb-3">
                            <div class="d-flex gap-2">
            ...
            $('#container-work_histories').html(`<span class="fs-11 text-muted fst-italic">Tekan tombol simpan setelah mengisi data</span>`);
        }
    })
    .catch((error) => {
        console.log(error);
...
const editWH = (wh_id, item_id) => {
    $('.alert').hide();
    $('#form-wh').attr('method', 'put').attr('action', 'work_history/'+wh_id);
...
const deleteWH = (wh_id) => {
  Swal.fire({
    ...
    if(result.isConfirmed) {
        var config = {
            method: 'delete', url: domain + 'work_history/' + wh_id,
        }
        axios(config)
        .then((response) => {
            fetchWH();
